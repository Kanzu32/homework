...
moveButton.addEventListener("click", ()=>{
    ...
    canvas1.addEventListener("click", chooseFirstDot);
...
function rotateMatrix(deg) { // против часовой
    let a = degToRad(deg);
    return rotateMatrixRad(a);
}

function rotateMatrixRad(rad) { // против часовой
...
function chooseFirstDot(event) {
    ...
    if (d != undefined) {
        inputCoords = d;
        canvas1.removeEventListener("click", chooseFirstDot);
        canvas1.addEventListener("click", chooseSecondDot);
    }
}

function chooseSecondDot(event) {
    if (event.offsetX < offset || event.offsetY < offset || event.offsetX > graphicsSize+offset || event.offsetY > graphicsSize+offset) {
        return;
    }
    let clickX = event.offsetX - offset;
    ...
    if (d != undefined) {
        inputCoords2 = d;
        canvas1.removeEventListener("click", chooseSecondDot);
        canvas1.addEventListener("click", setDot);
        redraw();
    }
}
...
function doTransform() {
    switch (mode) {
        case MODES.REGULAR:
            ...
            break;
        case MODES.MOVE:
            transformMatrix = moveMatrix(inputCoords2[0] - inputCoords[0], inputCoords2[1] - inputCoords[1]);
            break;
        case MODES.SCALE:
            ...
            let x = inputCoords2[0] - inputCoords[0];
            let y = inputCoords2[1] - inputCoords[1];
            ...
            transformMatrix = scaleMatrix(x*inputVal, y*inputVal);
            break;
        case MODES.REFLECT:
            let len = Math.sqrt((inputCoords[0]-inputCoords2[0])*(inputCoords[0]-inputCoords2[0]) + (inputCoords[1]-inputCoords2[1])*(inputCoords[1]-inputCoords2[1]));
            let angleXV = Math.acos((inputCoords2[0]-inputCoords[0])/len); // 1 0
            if (angleXV > Math.PI/2) angleXV = Math.PI-angleXV;
            console.log(len);
            console.log(inputCoords, inputCoords2);
            console.log(angleXV);
            transformMatrix = matrixMul(matrixMul(matrixMul(matrixMul(moveMatrix(-inputCoords[0], -inputCoords[1]), rotateMatrixRad(angleXV)), reflectMatrix()), rotateMatrixRad(-angleXV)), moveMatrix(inputCoords[0], inputCoords[1]));
            break;
        case MODES.ROTATE:
            inputVal = parseInt(angleField.value, 10);
            transformMatrix = rotateMatrix(inputVal);
            break;
    }
    if (isAnimated) {
